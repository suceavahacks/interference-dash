...
class Trampoline:
    def __init__(self, x, y, width=300, height=70):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
        ...
        try:
            self.image = pygame.image.load("assets/trampoline.png")
            self.image = pygame.transform.scale(self.image, (int(width), int(height)))
            self.image_loaded = True
        except Exception as e:
            print(f"Error loading trampoline image: {e}")
    ...
    def draw(self, screen):
        ...
        if self.image_loaded:
            screen.blit(self.image, (self.x, self.y + bounce_offset))
        else:
